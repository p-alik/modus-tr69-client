...
public class SampleScheduler implements Runnable, Observer {
	...
	public SampleScheduler(IParameterData pParameterData, String pBasePath) {
		...
		// get parameters
		reportSamplesParameter = parameterData.getParameter(basePath + "ReportSamples");
		sampleIntervalParameter = parameterData.getParameter(basePath + "SampleInterval");
		fetchSamplesParameter = parameterData.getParameter(basePath + "FetchSamples");
	...
	public void run() {
		...
		if (reportSample == -1) {
			reportSample = ((Long) reportSamplesParameter.getValue()).longValue();
		}
		long sampleInterval = ((Long) sampleIntervalParameter.getValue()).longValue();
		try {
			statusParameter.setValue("Enabled");

			for (long i = 0; i < reportSample; i++) {
				Log.debug("start " + i + "th sampleTask " + basePath);
				SampleTask sampleTask = new SampleTask(parameterData, basePath, i + 1);
				sampleTask.start();
	...
	private void discardAllValues() {
		...
		Parameter reportStartTimeParameter = parameterData.getParameter(basePath + "ReportStartTime");
		reportStartTimeParameter.setValueWithout("");

		// ReportEndTime parameter
		Parameter reportEndTimeParameter = parameterData.getParameter(basePath + "ReportEndTime");
		reportEndTimeParameter.setValueWithout("");

		// SampleSecondsParameter
		Parameter sampleSecondsParameter = parameterData.getParameter(basePath + "SampleSeconds");
		sampleSecondsParameter.setValueWithout("");

		// get ParameterNumberOfEntries parameter value
		Parameter parameterNumberOfEntriesParameter = parameterData.getParameter(basePath + "ParameterNumberOfEntries");
		long parameterNumberOfEntries = ((Long) parameterNumberOfEntriesParameter.getValue()).longValue();
		for (long i = 1; i <= parameterNumberOfEntries; i++) {
			Parameter sampleSeconds = parameterData.getParameter(basePath + "Parameter." + i + ".SampleSeconds");
			sampleSeconds.setValueWithout("");
			Parameter suspectData = parameterData.getParameter(basePath + "Parameter." + i + ".SuspectData");
			suspectData.setValueWithout("");
			Parameter valuesParameter = parameterData.getParameter(basePath + "Parameter." + i + ".Values");
			valuesParameter.setValueWithout("");
			Parameter failuresParameter = parameterData.getParameter(basePath + "Parameter." + i + ".Failures");
