...
public class DiscoveryImp extends Discovery {
	...
	private static final Long ROOT_DEVICE_MAXIMUM_NUMBER_OF_ENTRIES_VALUE = new Long(100);
	...
	// "Services.UPnP.Discovery.Device.";
	...
	public void initialize() throws Fault {
		...
		rootDeviceMaximumNumberOfEntries.addCheck(new CheckMaximum(4294967295L));
		rootDeviceMaximumNumberOfEntries.setValue(ROOT_DEVICE_MAXIMUM_NUMBER_OF_ENTRIES_VALUE);
		rootDeviceMaximumNumberOfEntries.setWritable(false);

		getParamDeviceNumberOfEntries().setStorageMode(StorageMode.COMPUTED);
		getParamRootDeviceNumberOfEntries().setStorageMode(StorageMode.COMPUTED);
		getParamServiceNumberOfEntries().setStorageMode(StorageMode.COMPUTED);
		getParamDeviceNumberOfEntries().setGetter(new GetterForList(deviceMap.values(), ParameterType.UINT));
		getParamRootDeviceNumberOfEntries().setGetter(new GetterForList(rootDeviceMap.values(), ParameterType.UINT));
		getParamServiceNumberOfEntries().setGetter(new GetterForList(serviceMap.values(), ParameterType.UINT));
	 ...
	 * <ul>
	 * <li>a branch exists, the status of the Device and the related services is
	 * updated to LeaseActive.
	 * </p>
	 * <li>no branch exists, a new one is created.</li> </ul> </p>
	 * 
	 * @param device
	 *            the device
	 */
	public void addUPnPDevice(UPnPDevice device) {
		...
		String parentUDN = (String) device.getDescriptions(null).get(UPnPDevice.PARENT_UDN);
		...
		if (parentUDN != null) {
			...
			if (deviceMap.containsKey(uuid)) {
				// update
				Log.debug("update status of " + uuid + ", status = LeaseActive");
			...
			if (rootDeviceMap.containsKey(uuid)) {
				...
				if (services != null) {
					for (int i = 0; i < services.length; i++) {
						...
						Log.debug("update status of the service " + usn + " to LeaseActive");
				...
				long rootDeviceNumberOfEntries = ((Long) getParamRootDeviceNumberOfEntries().getValue()).longValue();
				long rootDeviceMaximumNumberOfEntries = ((Long) this.rootDeviceMaximumNumberOfEntries.getValue())
						.longValue();
				if (rootDeviceNumberOfEntries < rootDeviceMaximumNumberOfEntries) {

					Log.debug("create datamodel branch for the root device " + uuid);
					RootDeviceImp rootDevice = new RootDeviceImp(data, getNextRootDevicePath(), device);
					try {
						rootDevice.initialize();
					} catch (Fault e1) {
						Log.error("unable to initialise the root device " + uuid, e1);
						return;
					}
					...
					// set RootDeviceNumberOfEntries parameter (for notification
					// purpose only)
					try {
						getParamRootDeviceNumberOfEntries().setValue(
								new Long((((Long) getParamRootDeviceNumberOfEntries().getValue()).longValue() + 1L)));
					} catch (Fault e1) {
					...
					if (services != null) {
						for (int i = 0; i < services.length; i++) {
							...
							ServiceImp serviceImp = new ServiceImp(data, getNextServicePath(), service, UUID + uuid);
							try {
								serviceImp.initialize();
							} catch (Fault e) {
								Log.error("unable to initialise the service " + usn, e);
								return;
							}
	...
	public static String computeUUID(UPnPDevice device) {
		return ((String) device.getDescriptions(null).get(UPnPDevice.UDN)).substring(UUID.length());
	}
	...
	public static String computeUSN(UPnPDevice upnpDevice, UPnPService service) {
		return upnpDevice.getDescriptions(null).get(UPnPDevice.UDN) + "::" + service.getType();
	}
	...
	// The following method getNextDevicePath() from the type DiscoveryImp is
	// never used
	// /**
