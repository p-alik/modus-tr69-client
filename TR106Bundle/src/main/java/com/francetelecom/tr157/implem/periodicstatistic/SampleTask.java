...
public class SampleTask implements Observer {
	...
	public SampleTask(IParameterData pParameterData, String pBasePath, long pSampleNumber) {
		parameterData = pParameterData;
		basePath = pBasePath;
		sampleNumber = pSampleNumber;
		sampleParameterTaskList = new ArrayList();

		parameterNumberOfEntriesParameter = parameterData.getParameter(basePath + "ParameterNumberOfEntries");
		reportStartTimeParameter = parameterData.getParameter(basePath + "ReportStartTime");
		reportEndTimeParameter = parameterData.getParameter(basePath + "ReportEndTime");
	...
	public void start() {
		...
		long numberOfEntries = ((Long) parameterNumberOfEntriesParameter.getValue()).longValue();
		for (long i = 1; i <= numberOfEntries; i++) {
			...
			SampleParameterTask sampleParameterTask = new SampleParameterTask(parameterData, parameterBasePath);
