...
public final class Activator implements BundleActivator {
	...
	private static final EventBehavior EVT_BEHAVIOR_M_DOWNLOAD = new EventBehavior(false, EventCode.ALWAYS_RETRY,
			TransferCompleteResponse.NAME);
	/** The Constant EVT_BEHAVIOR_TRANSFER_COMPLETE. */
	private static final EventBehavior EVT_BEHAVIOR_TRANSFER_COMPLETE = new EventBehavior(true, EventCode.ALWAYS_RETRY,
			TransferCompleteResponse.NAME);
	/** The rpc mng. */
	...
	public void start(final BundleContext context) throws Exception {
		...
		rpcMng.registerEventBehavior(EventCode.M_DOWNLOAD, EVT_BEHAVIOR_M_DOWNLOAD);
		rpcMng.registerEventBehavior(EventCode.TRANSFER_COMPLETE, EVT_BEHAVIOR_TRANSFER_COMPLETE);
		// Register new type of RPCMethod
		rpcMng.registerRPCMethod(Download.NAME);
		rpcMng.registerRPCMethod(Upload.NAME);
		rpcMng.registerRPCEncoder(UploadResponse.NAME, new UploadResponseEncoder());
		rpcMng.registerRPCEncoder(DownloadResponse.NAME, new DownloadResponseEncoder());
		engine = new Engine();
		...
		rpcMng.registerRPCEncoder(TransferComplete.NAME, new TransferCompleteEncoder());
		rpcMng.registerRPCDecoder(TransferCompleteResponse.NAME, new TransferCompleteResponseDecoder(engine));
	 ...
	 */
	private RPCMethodMngService getRPCMng(final BundleContext context) throws BundleException {
		...
		return (RPCMethodMngService) context.getService(rpcMethodMngServiceRef);
	}
