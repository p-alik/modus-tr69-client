...
public final class ApplyServiceListener implements ServiceListener {
	 ...
	 *            the context
	 */
	public ApplyServiceListener(final IEngine pEngine, final BundleContext pContext) {
		engine = pEngine;
		context = pContext;
		...
		try {
			...
			services = context.getServiceReferences(downloadProtocolName, null);
			if (services != null) {
				for (int i = 0; i < services.length; i++) {
					protocol = (IDownloadProtocol) context.getService(services[i]);
					if (protocol != null) {
	 ...
	 *            the ref
	 */
	private void applyActionServiceChanged(final ServiceEvent event, final ServiceReference ref) {
	 ...
	 *            the ref
	 */
	private void downloadProtocolServiceChanged(final ServiceEvent event, final ServiceReference ref) {
		IDownloadProtocol protocol = (IDownloadProtocol) context.getService(ref);
		switch (event.getType()) {
		case ServiceEvent.REGISTERED:
			lsServiceRefs.add(ref);
			engine.addDownloadProtocol(protocol);
