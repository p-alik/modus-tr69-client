...
public class SetParameterAttributesDecoder implements RPCDecoder {
	...
	public final RPCMethod decode(final Element element) throws Fault {
		...
		int index = eParameterList.indexOf("", "SetParameterAttributesStruct", 0);
		List lsESPAS = new ArrayList();
		while (index >= 0) {
			System.out.println("index" + index);
			lsESPAS.add(eParameterList.getChild(index));
			index = eParameterList.indexOf("", "SetParameterAttributesStruct", index + 1);
		}
		...
		while (it.hasNext()) {
			...
			sPAS.setNotificationChange("true".equals(value) || "Y".equals(value) || "1".equals(value));
			value = XMLUtil.extractValue(eSPA, "AccessListChange");
			sPAS.setAccessListChange("true".equals(value) || "Y".equals(value) || "1".equals(value));
			...
			while (index >= 0) {
				...
				index = eAccessList.indexOf("", "SetParameterAttributesStruct", index + 1);
			}
