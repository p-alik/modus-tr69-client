...
/**
 * The Class SetParameterAttributes. The specification is issue from the <a
 * href=
 * "http://www.broadband-forum.org/technical/download/TR-069Amendment2.pdf#page=51"
...
public final class SetParameterAttributes implements RPCMethod {
		...
		try {
			for (int i = 0; i < parameterList.length; i++) {
				...
				if (lsParameters == null || lsParameters.isEmpty()) {
					...
					error.append(": ");
					error.append(name);
					error.append(" does'nt represent any attribute" + " into current data model.");
	...
	/**
	 * In case of fault all parameters must be restore to their initial value to
	 * prevent the case of the parameter has been modify more than one time. The
	 * save list must be use from the end to the end.
	 * 
	 * @param lsSave
	 *            the ls save
	 */
	 ...
	 *             the fault
	 */
	protected static void setAttribute(final Parameter param, final SetParameterAttributesStruct setParam) throws Fault {
		if (setParam.isNotificationChange()) {
			if (param.isActiveNotificationDenied() & setParam.getNotification() == 2) {
				StringBuffer error;
	 ...
	 */
	public void setParameterList(final SetParameterAttributesStruct[] pParameterList) {
