...
public final class InformEncoder implements RPCEncoder {
	...
	public Element encode(final RPCMethod method) {
		...
		if (method instanceof Inform) {
			...
			eLsEvent.setAttribute(Soap.getSoapEncNameSpace(), "arrayType", "cwmp:EventStruct[" + size + "]");
			...
			eTime.addChild(Element.TEXT, Soap.convertDate2String(System.currentTimeMillis()));
			...
			Element eParameterList = eInform.createElement("", "ParameterList");
			eInform.addChild(Element.ELEMENT, eParameterList);
			size = inform.getParameterList().size();
			eParameterList.setAttribute(Soap.getSoapEncNameSpace(), "arrayType", "cwmp:ParameterValueStruct[" + size
					+ "]");
