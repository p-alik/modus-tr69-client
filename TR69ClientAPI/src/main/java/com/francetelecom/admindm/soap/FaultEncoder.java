...
public class FaultEncoder implements RPCEncoder {
	...
	public final Element encode(final RPCMethod method) throws Fault {
		...
		while (fault.getLsSetParamValuesFaults() != null && !fault.getLsSetParamValuesFaults().isEmpty()) {
			setParamValuesFault = (SetParamValuesFault) fault.getLsSetParamValuesFaults().remove(0);
			eCmpwFault.addChild(Element.ELEMENT, encoder.encode(setParamValuesFault));
		}
