...
public final class RPCMethodMng implements RPCMethodMngService {
	...
	private RPCMethodMng() {
		...
		registerRPCEncoder("SetParamValuesFault", new SetParamValuesFaultEncoder());
		registerRPCDecoder("Fault", new FaultDecoder());
		registerEventBehavior(EventCode.BOOTSTRAP, new EventBehavior(true, EventCode.DISCARD_OTHER_EVENTS,
				InformResponse.NAME));
		registerEventBehavior(EventCode.BOOT,
				new EventBehavior(true, EventCode.RETRY_UNTIL_REBOOT, InformResponse.NAME));
		registerEventBehavior(EventCode.PERIODIC, new EventBehavior(true, EventCode.ALWAYS_RETRY, InformResponse.NAME));
		registerEventBehavior(EventCode.SCHEDULED, new EventBehavior(true, EventCode.ALWAYS_RETRY, InformResponse.NAME));
		registerEventBehavior(EventCode.VALUE_CHANGE, new EventBehavior(true, EventCode.RETRY_UNTIL_REBOOT,
				InformResponse.NAME));
		// personal choice EventCode.Kicked never retry
		registerEventBehavior(EventCode.KICKED, new EventBehavior(true, EventCode.NEVER_RETRY, "KickedResponse"));
		registerEventBehavior(EventCode.CONNECTION_REQUEST, new EventBehavior(true, EventCode.NEVER_RETRY,
				InformResponse.NAME));
		registerEventBehavior(EventCode.DIAGNOSTICS_COMPLETE, new EventBehavior(true, EventCode.RETRY_UNTIL_REBOOT,
				InformResponse.NAME));
		// personal choice EventCode.RequestDownload never retry
		registerEventBehavior(EventCode.REQUEST_DOWNLOAD, new EventBehavior(true, EventCode.NEVER_RETRY,
		...
		registerEventBehavior(EventCode.DU_STATE_CHANGE_COMPLETE, new EventBehavior(true, EventCode.ALWAYS_RETRY,
				InformResponse.NAME));

		registerEventBehavior(EventCode.M_REBOOT, new EventBehavior(false, EventCode.ALWAYS_RETRY, InformResponse.NAME));
		registerEventBehavior(EventCode.M_SCHEDULE_INFORM, new EventBehavior(false, EventCode.ALWAYS_RETRY,
				InformResponse.NAME));
		// M_UPLOAD is optional
		// registerEventBehavior(EventCode.M_UPLOAD, new
		// EventBehavior(false,EventCode.ALWAYS_RETRY));
		registerEventBehavior(EventCode.M_CHANGE_DU_STATE, new EventBehavior(false, EventCode.ALWAYS_RETRY,
				InformResponse.NAME));
	}
	...
	public void registerRPCMethod(final String name) {
		...
		buffer.append(name);
		if (this.lsRPCMethods.contains(name)) {
			buffer.append(" failed: cause it was already registred");
			Log.warn(buffer.toString());
		} else {
			Log.debug(buffer.toString());
			this.lsRPCMethods.add(name);
		}
	}
	...
	public void registerRPCEncoder(final String name, final RPCEncoder encoder) {
		...
		buffer.append(name);
		Log.debug(buffer.toString());
		this.mapNameRPCEncoder.put(name, encoder);
	}
	...
	public void unregisterRPCEncoder(final String name) {
		...
		buffer.append(name);
		Log.debug(buffer.toString());
		this.mapNameRPCEncoder.remove(name);
	}
	...
	public void registerRPCDecoder(final String name, final RPCDecoder decoder) {
		...
		buffer.append(name);
		Log.debug(buffer.toString());
		this.mapNameRPCDecoder.put(name, decoder);
	}
	...
	public void unregisterRPCDecoder(final String name) {
		...
		buffer.append(name);
		Log.debug(buffer.toString());
		this.mapNameRPCDecoder.remove(name);
	}
	 ...
	 *            the event behavior
	 */
	public void registerEventBehavior(final String name, final EventBehavior eventBehavior) {
		StringBuffer buffer = new StringBuffer("Register EventBehavior ");
		buffer.append(name);
		Log.debug(buffer.toString());
		this.mapEventnameEventBehavior.put(name, eventBehavior);
	}
