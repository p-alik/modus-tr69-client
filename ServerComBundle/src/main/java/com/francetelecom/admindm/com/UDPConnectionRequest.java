...
public final class UDPConnectionRequest {
	...
	private static final String CONNECTION_REQUEST_USERNAME = "ManagementServer.ConnectionRequestUsername";
	/** ConnectionRequestPassword parameter name. */
	private static final String CONNECTION_REQUEST_PASSWORD = "ManagementServer.ConnectionRequestPassword";
	/** data model. */
	 ...
	 *            the parameter data
	 */
	public UDPConnectionRequest(final IParameterData pParameterData, final DatagramPacket packet) {
	...
	private boolean validate() {
		...
		if (!GET.equals(method)) {
			Log.debug("invalid method, should be " + GET + " instead of " + method);
			return false;
		}
		// request uri parsing
		String requestUri = stringTokenizer.nextToken();
		requestURI = new RequestURIParser(requestUri);
		UDPConnectionRequest lastUDPConnectionRequest = UDPServer.getInstance().getLastUDPConnectionRequest();
		// protocol
		String protocol = stringTokenizer.nextToken();
		if (!PROTOCOL.equals(protocol)) {
			Log.debug("invalid protocol, should be " + PROTOCOL + " instead of " + protocol);
			return false;
		}
		...
		try {
			...
			Log.error("unable to extract the current TS attribute : " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		...
		if (lastUDPConnectionRequest != null) {
			try {
				...
				Log.error("previousTs extraction should not throw an Exception", e);
			}
		}
		if (currentTs <= previousTs) {
			Log.debug("invalid ts (must be strictly greater than the ts " + "of the previous udp connection request");
			return false;
		}
		...
		if (lastUDPConnectionRequest != null) {
			try {
				long previousId = getId(lastUDPConnectionRequest.getRequestUri());
		...
		try {
			Parameter connectionRequestUsernameParameter = parameterData.createOrRetrieveParameter(parameterData
					.getRoot() + CONNECTION_REQUEST_USERNAME);
			connectionRequestUsernameString = connectionRequestUsernameParameter.getTextValue(null);
		} catch (Fault e) {
		...
		if (un == null) {
			...
			Log.error("invalid un attribute value. It is equal to " + un + " but should be equal to "
	...
	private boolean authenticate() {
		...
		try {
			Parameter connectionRequestPasswordParameter = parameterData.createOrRetrieveParameter(parameterData
					.getRoot() + CONNECTION_REQUEST_PASSWORD);
			...
			mac.init(signinKey);
			// compute value
			String text = requestURI.getAttribute(RequestURIParser.TS) + requestURI.getAttribute(RequestURIParser.ID)
					+ requestURI.getAttribute(RequestURIParser.UN) + requestURI.getAttribute(RequestURIParser.CN);
			Log.debug("text = " + text);
	 ...
	 * @throws Exception
	 *             if any error occurs such as the requestUri parameter is null
	 *             or the ts attribute could not be parsed
	 */
	private static long getTs(final RequestURIParser requestUri) throws Exception {
		if (requestUri != null) {
	 ...
	 * @throws Exception
	 *             if any error occurs (requestUri parameter null or id
	 *             parameter not found.
	 */
	private static long getId(final RequestURIParser requestUri) throws Exception {
		if (requestUri != null) {
