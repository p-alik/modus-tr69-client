	...
	public void run() {
		...
		if ((data != null) && (data.length > 0)) {
			if ((data[0] == 0) || (data[0] == 1)) {
				...
				ISTUNCLient stunClient = UDPServer.getInstance().getSTUNClient();
				if (stunClient != null) {
					stunClient.addMessage(data);
				}
			} else {
				String dataString = new String(data);
				if ((dataString != null) && (Character.isLetter(dataString.charAt(0)))) {
					Log.debug("UDP Connection Request from the server");
					UDPServer.getInstance().getUDPConnectionRequestQueue().add(datagramPacket);
				} else {
