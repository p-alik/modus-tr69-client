...
public final class GetParameterNames implements RPCMethod {
	 ...
	 * 
	 * @param pParameterPath
	 *            the new parameter path
	 */
	...
	public void perform(final Session session) throws Fault {
		...
		parameterList = getArrayOfParameterInfo(parameterPath, nextLevel, session.getParameterData());
	 ...
	 */
	protected static ParameterInfoStruct[] getArrayOfParameterInfo(final String name, final boolean nextLevel,
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (isSelectable(temp.getName(), name, nextLevel)) {
				pis = new ParameterInfoStruct(temp.getName(), temp.isWritable());
				lsParameter.add(pis);
			}
		}
		result = (ParameterInfoStruct[]) lsParameter.toArray(new ParameterInfoStruct[lsParameter.size()]);
		return result;
	}
	 ...
	 */
	protected static boolean isSelectable(final String initial, final String name, final boolean nextLevel) {
