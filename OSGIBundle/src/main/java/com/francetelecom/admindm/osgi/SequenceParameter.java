...
public class SequenceParameter implements Setter {
	...
	public SequenceParameter(final BundleContext pBundleContext, final IParameterData pParameterData) {
		bundleContext = pBundleContext;
		parameterData = pParameterData;

		try {
			Parameter startSequenceParameter = parameterData.createOrRetrieveParameter(parameterData.getRoot()
			...
			Parameter stopSequenceParameter = parameterData.createOrRetrieveParameter(parameterData.getRoot()
			...
			Parameter uninstallSequenceParameter = parameterData.createOrRetrieveParameter(parameterData.getRoot()
			...
			Log.error("Unable to create and initialize the *Sequence parameter", e);
		}
	}
			...
			for (int i = 0; i < locations.length; i++) {
				...
				if (bundle != null) {
					try {
						...
						// nothing to do
						Log.info("unable to uninstall the bundle " + bundle.getBundleId() + " - "
								+ bundle.getLocation());
					}
				}
			}
			...
			for (int i = 0; i < locations.length; i++) {
				...
				if (bundle != null) {
					try {
						...
						// nothing to do
						Log.info("unable to stop the bundle " + bundle.getBundleId() + " - " + bundle.getLocation());
					}
				}
			}
		}
		...
		if (locations != null) {
			...
			for (int i = 0; i < locations.length; i++) {
				...
				if (bundle != null) {
					...
					throw new Fault(9007, "The location " + locations[i] + " doesn't match with a bundle !!!");
				}
			}
			...
			for (Iterator it = bundleInfoList.iterator(); it.hasNext();) {
				...
				try {
					...
					String error = "Unable to start the bundle " + bundleInfo.getBundleLocation();
