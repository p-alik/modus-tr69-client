...
public class ParameterDataServiceListener implements ServiceListener {
	/** The Constant filter. */
	public static final String filter = "(objectclass=" + IParameterData.class.getName() + ")";
		...
		if (service instanceof IParameterData) {
			...
			case ServiceEvent.REGISTERED:
				try {
					...
					SequenceParameter startSequenceParameter = new SequenceParameter(context, data);
					Log.debug("startSequenceParameter: " + startSequenceParameter);
	 ...
	 *            the parameter data
	 */
	private static void createOSGIRootParameter(IParameterData parameterData) throws Fault {
		Parameter osgi = parameterData.createOrRetrieveParameter(parameterData.getRoot() + OSGIBundleListener.osgiPath);
		osgi.setType(ParameterType.ANY);
		osgi.setWritable(false);
		osgi.setValue("");
		osgi.setStorageMode(StorageMode.COMPUTED);

		Parameter bundle = parameterData.createOrRetrieveParameter(parameterData.getRoot()
